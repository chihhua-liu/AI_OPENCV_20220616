...
img = cv2.imread('images/mybaby.jpg',0)

cv2.imshow('image',img)
cv2.waitKey(0)        # 按下任意鍵，destroyAllWindows
...
plt.show()
-----------------------------------------------------------------
# channel rch = img[:,:,2]------ np.dstack((bch,gch,rch))-----cv.imwrite-----------------
import numpy as np
import cv2 as cv
# Load an color image
...
cv.imshow('Blue',bch)
cv.waitKey(0)
# 沿深度方向（沿第三轴）按顺序Stack(疊成堆)数组。
bgr = np.dstack((bch,gch,rch))
cv.imwrite('images/scenery_stack.jpg',bgr)
cv.imshow('BGR',bgr)
cv.waitKey(0)
